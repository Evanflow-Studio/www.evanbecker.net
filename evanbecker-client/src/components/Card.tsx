...
Card.Link = function CardLink({
    ...
    return (
        <>
            <div className="absolute -inset-x-4 -inset-y-6 z-0 scale-95 opacity-0 transition group-hover:scale-100 group-hover:opacity-100 bg-slate-800/50 sm:-inset-x-6 sm:rounded-2xl" />
            <Link {...props}>
...
Card.Eyebrow = function CardEyebrow<T extends React.ElementType = 'p'>({
    ...
    return (
        <Component
            className={clsx(
                className,
                'relative z-10 order-first mb-3 flex items-center text-sm text-slate-500',
                decorate && 'pl-3.5',
            )}
            {...props}
        >
