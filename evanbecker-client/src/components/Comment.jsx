...
function ReplyBox({
  ...
  const [commentText, setCommentText] = useState(
    isChild ? `@${comment.author.firstName} ` : '',
  )
  ...
  const addReply = async () => {
    try {
      ...
      const call = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}api/v1/comment/${targetLocation}/reply/${id}`,
        {
          method: 'POST',
          headers: {
...
export function Comment({
  ...
  const deleteComment = async () => {
    try {
      ...
      const call = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}api/v1/comment/${comment.id}`,
        {
          method: 'DELETE',
          headers: {
            Authorization: `Bearer ${accessToken}`,
      ...
      <>
        <div className={IndentLevel(isChild ? 2 : 1)}>
          <div className="mt-2 mb-2 pt-2 pb-2">
            <article
              className={
                comment && (comment.author.isAdmin || comment.author.isOwner)
                  ? 'rounded-lg bg-slate-700 p-4 text-base'
                  : 'rounded-lg p-4 text-base'
              }
            >
              <footer className="mb-2 flex items-center justify-between">
                <div className="flex items-center">
                  <p className="mr-3 inline-flex items-center text-sm font-semibold text-gray-900 text-slate-200">
                    <img
              ...
              <div className="mt-4 flex items-center space-x-4">
                <button
                  type="button"
                  className="flex items-center text-sm font-medium text-slate-400 hover:underline"
                  onClick={() => {
            ...
            {clickToggled && (
              <ReplyBox
                id={isChild ? parentId : comment.id}
                parentComment={parentComment}
                comment={comment}
                isChild={isChild}
                handleParentReply={(comment) => {
                  setClickToggled(false)
